...
***** Tracing RPC request
****** A helloword microservice application
[] Inter-Process Context Propagation
In order to continue the trace over the process boundaries and RPC calls,
we need a way to propagate span context over wire. So we will introduce
Inject and Extract.
